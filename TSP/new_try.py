...
def generate_same_cities(amount):
    ...
    cities = np.array([(x.rvs(), y.rvs()) for i in range(amount)])
    return cities
...
def generate_slightly_different_cities(cities):
    ...
    for i in cities:
        row = [(j + alfa_distr.rvs(), 3) for j in i]
...
def compute_distances(cities):
    ...
    for i in cities:
        row = [distance.euclidean(i, j, 3) for j in cities]
        row_array = np.array(row)  # needs to be an np.array
...
def training():
    ...
    amount_instances = 2
    amount_cities = 5
    training_instances = random_instance_generator(amount_instances,
                                            amount_cities)  # genereer a keer b steden die lichtjes anders zijn
...
def miplearnSolver():
    ...
    ##-------------------------TEST DATA--------------------##
    test_instances = random_instance_generator(1, 20)[0]
...
def regularSolver():
    test_instance = random_instance_generator(1, 20)[0]
